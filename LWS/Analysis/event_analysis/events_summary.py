...
def __get_fixation_summary_dict(fixations: List[LWSFixationEvent]) -> Dict[str, float]:
    ...
    data["mean_visual_angle_to_closest_target"] = float(np.nanmean([f.visual_angle_to_targets for f in fixations]))
    data["std_visual_angle_to_closest_target"] = float(np.nanstd([f.visual_angle_to_targets for f in fixations]))
    return data
