...
def __get_fixation_summary_dict(fixations: List[LWSFixationEvent]) -> Dict[str, float]:
    ...
    data["distance_to_target_mean"] = np.nanmean(
        [f.distance_to_target for f in fixations if np.isfinite(f.distance_to_target)])
    data["distance_to_target_std"] = np.nanstd(
        [f.distance_to_target for f in fixations if np.isfinite(f.distance_to_target)])
    return data
