...
def monocular_blink_detection(timestamps: np.ndarray, x: np.ndarray, y: np.ndarray,
                              missing_value: float = cnst.MISSING_VALUE,
                              time_between_blinks: float = 20,
                              min_duration: float = 50) -> np.ndarray:
    """
    ...
    :param missing_value: value that indicates missing data
    :param time_between_blinks: maximum duration between blink-candidates, required to merge them together
    :param min_duration: minimum duration of a blink in milliseconds
    ...
    max_length_between_candidates = u.calculate_minimum_sample_count(time_between_blinks, sr)
    candidate_start_end_idxs = _find_blink_candidates(x, y, missing_value, max_length_between_candidates)

    # exclude blinks that are too short
    min_length_for_blink = u.calculate_minimum_sample_count(min_duration, sr)
    blink_start_end_idxs = [(start, end) for start, end in candidate_start_end_idxs if end - start >= min_length_for_blink]
...
def _find_blink_candidates(x: np.ndarray, y: np.ndarray,
                           missing_value: float, merge_threshold: int) -> List[tuple]:
    """
    ...
    :param merge_threshold: maximum distance between two missing data periods to merge them together
    ...
    is_missing = np.logical_or(x == missing_value, y == missing_value)
